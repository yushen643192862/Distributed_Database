...
本阶段目标是先跑通一个最小可用系统。它在一�?JVM 进程里模�?Coordinator、MetaStore 和多�?DataNode，用来展示“元数据管理 + 分片路由 + 节点本地执行 + 结果聚合”的核心思路�?
...
- `DataNode`：模拟数据节点�?
- `LocalMiniSqlEngine`：每�?DataNode 内部的本地存储执行层�?
...
- SQL 子集�?
  ...
  - `SHOW SHARDS`
- 表级主键唯一性检查�?
- 非分片键查询/更新/删除会广播到所�?shard，分片键等值条件会只访问目�?shard�?
...
- Step 06：扩�?SQL 解析器，支持投影列、更多比较条件和简单索引�?
